...
const closeLogic = async (bot, response, timeframe, order) => {
        ...
        if (item.conditional === ">") {
            if (item.indicator === "MA(10)") {
                if (!item.signal) {
                    ...
                    close_logic.push({ index: i, result: result, addConditional: item.addConditional })
                } else {
    ...
    function checkCandlePattern(i, item, previous) {
        if (item.conditional === "true") {
            ...
            close_logic.push({ index: i, result: Boolean(result), addConditional: item.addConditional })
        } else {
    ...
    for (const [i, item] of bot.close_logic.entries()) {
        ...
        if (item.indicator === "Price Increased") {
            if (item.conditional === "buy") {
                const percent = Number(order.buy_price) + (Number(item.value) / 100) * Number(order.buy_price)
    ...
    const result = { logic: conditionLogic(close_logic), price: current_price }

    return result
}

module.exports = closeLogic